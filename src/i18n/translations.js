export const DIRECTIONS = { he: 'rtl', en: 'ltr', ar: 'rtl' }

// Fixed, app-wide vocabulary for allergen statement types — shown as a group
// heading above that group's icon grid. A future statement type is added
// here (×3 languages) plus one entry in ALLERGEN_STATEMENT_ORDER; no product
// config or component code needs to change. ALLERGEN_STATEMENT_COLORS is the
// same kind of fixed vocabulary (not per-product) — every product gets the
// same color per statement type.
export const ALLERGEN_STATEMENT_ORDER = ['contains', 'mayContain', 'doesNotContain']

export const ALLERGEN_STATEMENT_COLORS = {
  contains: '#EA2427',
  mayContain: '#F5851F',
  doesNotContain: '#43AA49',
}

export const translations = {
  he: {
    logoAlt: 'קריא למאכל',
    weightLabel: 'משקל נקי כולל',
    scanMore: 'סרוק מוצר נוסף',
    qrScannerInstructions: 'כוונו את המצלמה לקוד ה-QR שעל האריזה',
    qrScannerNotFound: 'הקוד שנסרק אינו מזוהה',
    qrScannerError: 'לא ניתן לגשת למצלמה. יש לאשר הרשאת מצלמה ולנסות שוב.',
    qrScannerKeepScanning: 'המשך לסרוק',
    moreInfo: 'מידע נוסף',
    back: 'חזרה',
    close: 'סגור',
    categoryPlaceholder: 'התוכן של קטגוריה זו בבנייה, יעודכן בקרוב.',
    fontGlyph: 'א',
    contrast: 'ניגודיות',
    listen: 'השמעה',
    listening: 'מקריא...',
    shrinkText: 'הקטן טקסט',
    growText: 'הגדל טקסט',
    speechLang: 'he-IL',
    ingredientsTitle: 'רשימת רכיבים',
    allergenStatements: {
      contains: 'המוצר מכיל',
      mayContain: 'המוצר עלול להכיל',
      doesNotContain: 'המוצר אינו מכיל',
    },
    nutritionTitle: 'סימון תזונתי',
    kosherTitle: 'כשרות',
    manufacturerTitle: 'פרטי יצרן/יבואן/משווק',
    storageTitle: 'הוראות אחסון',
    recyclingTitle: 'מיחזור',
    warningsTitle: 'אזהרות',
    categories: {
      ingredients: 'רשימת הרכיבים',
      allergens: 'מידע על אלרגנים',
      kosher: 'כשרות',
      nutrition: 'סימון\nתזונתי',
      storage: 'הוראות אחסון',
      manufacturer: 'פרטי יצרן/יבואן/משווק',
      recycling: 'מיחזור',
      warnings: 'אזהרות',
    },
  },
  en: {
    logoAlt: 'Readable for Food',
    weightLabel: 'Total net wight',
    scanMore: 'Scan another product',
    qrScannerInstructions: 'Point your camera at the QR code on the package',
    qrScannerNotFound: "That code wasn't recognized",
    qrScannerError: 'Camera access is unavailable. Please allow camera permission and try again.',
    qrScannerKeepScanning: 'Keep scanning',
    moreInfo: 'More info',
    back: 'Back',
    close: 'Close',
    categoryPlaceholder: "This category's content is under construction, coming soon.",
    fontGlyph: 'A',
    contrast: 'Contrast',
    listen: 'Listen',
    listening: 'Reading...',
    shrinkText: 'Decrease text size',
    growText: 'Increase text size',
    speechLang: 'en-US',
    ingredientsTitle: 'Ingredients',
    allergenStatements: {
      contains: 'Contains',
      mayContain: 'May contain',
      doesNotContain: 'Does not contain',
    },
    nutritionTitle: 'Nutrition facts',
    kosherTitle: 'Kosher',
    manufacturerTitle: 'Manufacturer/Importer/\nDistributor details',
    storageTitle: 'Storage instructions',
    recyclingTitle: 'Recycling',
    warningsTitle: 'Warnings',
    categories: {
      ingredients: 'Ingredients list',
      allergens: 'Allergen info',
      kosher: 'Kosher',
      nutrition: 'Nutrition\nfacts',
      storage: 'Storage instructions',
      manufacturer: 'Manufacturer/Importer/\nDistributor details',
      recycling: 'Recycling',
      warnings: 'Warnings',
    },
  },
  ar: {
    logoAlt: 'قارئ للطعام',
    weightLabel: 'إجمالي الوزن الصافي',
    scanMore: 'امسح منتجًا آخر',
    qrScannerInstructions: 'وجّه الكاميرا نحو رمز QR الموجود على العبوة',
    qrScannerNotFound: 'الرمز الممسوح غير معروف',
    qrScannerError: 'تعذّر الوصول إلى الكاميرا. يرجى السماح بإذن الكاميرا والمحاولة مرة أخرى.',
    qrScannerKeepScanning: 'متابعة المسح',
    moreInfo: 'معلومات إضافية',
    back: 'رجوع',
    close: 'إغلاق',
    categoryPlaceholder: 'محتوى هذه الفئة قيد الإنشاء، سيتم التحديث قريبًا.',
    fontGlyph: 'أ',
    contrast: 'التباين',
    listen: 'استماع',
    listening: 'جارٍ القراءة...',
    shrinkText: 'تصغير النص',
    growText: 'تكبير النص',
    speechLang: 'ar-SA',
    ingredientsTitle: 'المكونات',
    allergenStatements: {
      contains: 'يحتوي على',
      mayContain: 'قد يحتوي على',
      doesNotContain: 'لا يحتوي على',
    },
    nutritionTitle: 'القيمة الغذائية',
    kosherTitle: 'كوشر',
    manufacturerTitle: 'تفاصيل المصنّع/المستورد/الموزّع',
    storageTitle: 'تعليمات التخزين',
    recyclingTitle: 'إعادة التدوير',
    warningsTitle: 'تحذيرات',
    categories: {
      ingredients: 'قائمة المكونات',
      allergens: 'معلومات عن الحساسية',
      kosher: 'كوشر',
      nutrition: 'القيمة\nالغذائية',
      storage: 'تعليمات التخزين',
      manufacturer: 'تفاصيل المصنّع/المستورد/الموزّع',
      recycling: 'إعادة التدوير',
      warnings: 'تحذيرات',
    },
  },
}
